...
8. `egm-gpu-to-rack` - implements all tests from `egm` suite in the more scalable `gpu-to-rack` fashion
...
When benchmarking simultaneous copies, nvloom will make sure that all copy streams stay loaded until all measured copies finish.

Once the measured copy on a stream ends, nvloom adds one iteration at a time to the stream in a double-buffering fashion to eliminate any gaps in execution.

Load sustainment is only enabled when there are 2 or more simultaneous copies.

Load sustainment is NOT supported for:
- latency measurements
- any multicast reduction operation

When a non-supported operation is combined with a supported operation, load sustainment will trigger for supported operations.
...
The argument is interpreted as number of bytes. Also accepts k, m, g, t suffixes for kibibytes, mebibytes, gibibytes and tebibytes respectively. Letter case is ignored. Byte count must be divisible by 128 bytes.

The default value is 512 mebibytes
...
The default is 3 samples per rack.
...
The heatmap plotter is a Python3 application. We recommend running it through `uv` to automatically maintain the dependencies. If `uv` is installed on your system, you can simply run `./plot_heatmaps.py`. If not, you can do `python3 plot_heatmaps.py`, but you will have to manually install the dependencies. The list of dependencies can be found in `plot_heatmaps.py` in a PEP 723 compliant format.
...
The `--filename_prefix` option allows you to specify a string that will be prepended to the filenames of all output heatmap images generated by `plot_heatmaps.py`.

## Histogram options

By default, `plot_heatmaps.py` generates heatmap visualizations. You can enable histogram mode instead using the `--histogram` flag. When histogram mode is enabled, the data from each testcase is flattened into a single distribution and displayed as a histogram rather than a 2D heatmap.

### --histogram

Enables histogram mode instead of heatmap mode. When this flag is set, `plot_heatmaps.py` will generate histogram plots showing the distribution of all data values from each testcase.

Controlled with `--histogram` flag (boolean, default: False).

### --histogram_dynamic_min

Controls whether the histogram x-axis range dynamically adjusts to start at the minimum data value. When enabled, the histogram x-axis will start at the minimum value found in the data. When disabled (default), the histogram x-axis will always start at 0, regardless of the minimum value in the data.

Controlled with `--histogram_dynamic_min` flag (boolean, default: False).

### --histogram_bins

Controls the number of bins used in the histogram. More bins provide finer granularity but may make the histogram harder to read if there are too many. Fewer bins provide a smoother, more generalized view of the distribution.

The default is 20 bins.

Controlled with `--histogram_bins` option.
...
For `multicast` the copy is executed by a custom multicast memcpy kernel. In `multicast` case, the source memory has to be either `device` or `egm`, and the destination memory has to be `multicast`. Moreover, the copy type has to be `write`. Nvloom allocates multicast memory on every single process/GPU, so multicast copies will be writing to every single GPU visible to nvloom.
...
You can also use `k`, `m`, `g` and `t` suffixes, meaning `1024`, `1024*1024`, `1024*1024*1024` and `1024*1024*1024*1024`. For example, `10k` will be evaluated to 10240 bytes.
